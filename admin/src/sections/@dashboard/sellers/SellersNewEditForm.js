...
SellersNewEditForm.propTypes = {
    isEdit: PropTypes.bool,
    currentService: PropTypes.object,
};

export default function SellersNewEditForm({ isEdit = false, currentService }) {
    const navigate = useNavigate();

    const { enqueueSnackbar } = useSnackbar();

    const NewSellersSchema = Yup.object().shape({
        title: Yup.object({
    ...
    const methods = useForm({
        resolver: yupResolver(NewSellersSchema),
        defaultValues,
    });
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            navigate(PATH_DASHBOARD.sellers.list);
                ...
                <Grid item xs={12} md={8}>
                    <Card sx={{ p: 3 }}>
                        ...
                        <Stack alignItems="flex-end" sx={{ mt: 3 }}>
                            ...
                            >
                                {!isEdit ? "Create Sellers" : "Save Changes"}
                            </LoadingButton>
                        </Stack>
                    </Card>
                </Grid>
