...
CoinsNewEditForm.propTypes = {
    isEdit: PropTypes.bool,
    currentService: PropTypes.object,
};

export default function CoinsNewEditForm({ isEdit = false, currentService }) {
    const navigate = useNavigate();

    const { enqueueSnackbar } = useSnackbar();

    const NewCoinsSchema = Yup.object().shape({
        title: Yup.object({
    ...
    const methods = useForm({
        resolver: yupResolver(NewCoinsSchema),
        defaultValues,
    });
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            navigate(PATH_DASHBOARD.coins.list);
                ...
                <Grid item xs={12} md={8}>
                    <Card sx={{ p: 3 }}>
                        ...
                        <Stack alignItems="flex-end" sx={{ mt: 3 }}>
                            ...
                            >
                                {!isEdit ? "Create Coins" : "Save Changes"}
                            </LoadingButton>
                        </Stack>
                    </Card>
                </Grid>
