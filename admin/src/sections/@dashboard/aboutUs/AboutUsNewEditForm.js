...
AboutUsNewEditForm.propTypes = {
    isEdit: PropTypes.bool,
    currentService: PropTypes.object,
};

export default function AboutUsNewEditForm({ isEdit = false, currentService }) {
    const navigate = useNavigate();

    const { enqueueSnackbar } = useSnackbar();

    const NewAboutUsSchema = Yup.object().shape({
        title: Yup.object({
    ...
    const methods = useForm({
        resolver: yupResolver(NewAboutUsSchema),
        defaultValues,
    });
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            navigate(PATH_DASHBOARD.aboutUs.list);
                ...
                <Grid item xs={12} md={8}>
                    <Card sx={{ p: 3 }}>
                        ...
                        <Stack alignItems="flex-end" sx={{ mt: 3 }}>
                            ...
                            >
                                {!isEdit ? "Create AboutUs" : "Save Changes"}
                            </LoadingButton>
                        </Stack>
                    </Card>
                </Grid>
